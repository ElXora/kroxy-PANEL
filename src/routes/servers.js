...
router.post('/create', async (req, res) => {
  ...
  // Use CF hostname > FQDN > connectIp > ip (handles localhost/0.0.0.0 correctly)
  const connectHost = (node.cfEnabled && node.cfPublicHostname) ? node.cfPublicHostname : (node.fqdn || node.connectIp || node.ip || 'localhost');
...
// Startup variables save
router.post('/:id/startup', async (req, res) => {
  const server = await db.servers.findOne({ _id: req.params.id, userId: res.locals.user._id });
  if (!server) { req.flash('error', 'Not found.'); return res.redirect('/servers'); }
  const { version, serverType, ram, javaFlags, onlineMode } = req.body;
  await db.servers.update({ _id: server._id }, { $set: {
    version, serverType: serverType || 'VANILLA',
    ram: parseInt(ram) || server.ram,
    javaFlags: javaFlags || '',
  ...
  if (server.containerId) await docker.removeContainer(server.containerId);
  const fresh = await db.servers.findOne({ _id: server._id });
  docker.createMinecraftServer(fresh).then(async r => {
    if (r.success) await db.servers.update({ _id: server._id }, { $set: { containerId: r.containerId, status: 'offline' } });
  });
  req.flash('success', 'Startup variables saved. Container is being recreated.');
  res.redirect('/servers/' + server._id);
});
...
router.post('/:id/settings', async (req, res) => {
  ...
  const portNum = parseInt(port);
  if (portNum !== server.port) {
    const inUse = await db.servers.findOne({ port: portNum, _id: { $ne: server._id } });
    if (inUse) { req.flash('error', 'Port already in use.'); return res.redirect('/servers/' + server._id); }
  }
  // Rebuild connect address
  const node = server.nodeId ? await db.nodes.findOne({ _id: server.nodeId }) : null;
  const host = node ? ((node.cfEnabled && node.cfPublicHostname) ? node.cfPublicHostname : (node.connectIp || node.fqdn || node.ip)) : 'localhost';
  await db.servers.update({ _id: server._id }, { $set: {
    name: name.trim().substring(0,32),
    description: description || '',
    port: portNum,
    connectAddress: `${host}:${portNum}`,
    updatedAt: new Date(),
  }});
...
router.post('/:id/reinstall', async (req, res) => {
  ...
  if (!server) return res.redirect('/servers');
  if (server.containerId) await docker.removeContainer(server.containerId);
  await db.servers.update({ _id: server._id }, { $set: { containerId: null, status: 'installing' } });
  docker.createMinecraftServer(server).then(async r => {
    if (r.success) await db.servers.update({ _id: server._id }, { $set: { containerId: r.containerId, status: 'offline' } });
    else await db.servers.update({ _id: server._id }, { $set: { status: 'error' } });
  });
  req.flash('success', 'Server is being reinstalled.');
  res.redirect('/servers/' + server._id);
});
...
router.post('/:id/kill', async (req, res) => {
  const server = await db.servers.findOne({ _id: req.params.id, userId: res.locals.user._id });
  if (!server || !server.containerId) return res.json({ success: false });
  const ok = await docker.killContainer(server.containerId);
  if (ok) await db.servers.update({ _id: server._id }, { $set: { status: 'offline' } });
  res.json({ success: ok });
});

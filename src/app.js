...
app.use(async (req, res, next) => {
  ...
  if (req.session.userId) {
    try {
      res.locals.user = await db.users.findOne({ _id: req.session.userId });
      // Generate/refresh WS token for this session
      if (res.locals.user) {
        const crypto = require('crypto');
        if (!res.locals.user.wsToken) {
          const tok = crypto.randomBytes(24).toString('hex');
          await db.users.update({ _id: res.locals.user._id }, { $set: { wsToken: tok } });
          res.locals.user.wsToken = tok;
        }
        res.locals.wsToken = res.locals.user.wsToken;
      }
    } catch (e) {}
  }
  next();
...
app.use('/servers',    requireAuth, schedulesRoutes);
app.use('/servers',    requireAuth, subusersRoutes);

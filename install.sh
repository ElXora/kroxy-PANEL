...
# Add kroxy user to docker group so it can control containers
...
cat > /etc/systemd/system/kroxy-panel.service << SERVICE
[Unit]
...
systemctl enable kroxy-panel
systemctl start kroxy-panel
sleep 2

if systemctl is-active --quiet kroxy-panel; then
  echo -e "${GREEN}    Panel service started successfully.${NC}"
else
  echo -e "${RED}    Panel failed to start. Check: journalctl -u kroxy-panel -f${NC}"
fi
...
cat > /etc/nginx/sites-available/kroxy-panel << NGINX
server {
    listen 80;
    server_name $DOMAIN;
...
ln -sf /etc/nginx/sites-available/kroxy-panel /etc/nginx/sites-enabled/kroxy-panel
...
echo ""
echo -e "${GREEN}╔════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║     Kroxy Panel Install Complete!     ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════╝${NC}"
echo ""
...
echo -e "  ${CYAN}systemctl status kroxy-panel${NC}    — check status"
echo -e "  ${CYAN}systemctl restart kroxy-panel${NC}   — restart"
echo -e "  ${CYAN}journalctl -u kroxy-panel -f${NC}    — live logs"
echo ""
